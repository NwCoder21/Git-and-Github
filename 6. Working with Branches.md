...
Now I've switched, if I run `git log`, only thing that has changed here is that `HEAD` now points to `georges` branch and I have all those commits from the earlier `old_songs` branch on `georges`, so now I can do a bit more work.
